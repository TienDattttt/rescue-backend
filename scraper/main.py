...
def _fetch_comments_for_post_legacy(post_id, cookies=None):
...
def fetch_comments_for_post(post_id, cookies=None, post_url=None):
    """Fetch all comments and replies for a given post_id."""
    feedback_id = convert_post_id_to_feedback_id(post_id)
    print(f"  Fetching comments for post {post_id}...")
    print(f"  Using feedback_id: {feedback_id}")
    ...
    comments = []
    post_info = None
    selenium_fallback_reason = None
    disable_selenium = os.getenv("DISABLE_SELENIUM_COMMENTS", "").strip().lower() in {"1", "true", "yes", "on"}

    if post_url and not disable_selenium:
        try:
            from selenium_comment_scraper import fetch_comments_with_selenium

            print(f"  Trying SeleniumBase comment capture for {post_url}...")
            comments, post_info = fetch_comments_with_selenium(post_url, cookies=cookies, post_id=post_id)
            if comments:
                print(f"  SeleniumBase captured {len(comments)} top-level comments before reply expansion")
                if post_info is None:
                    post_info = {}
                post_info["comment_source"] = "seleniumbase_network"
            else:
                selenium_fallback_reason = "SeleniumBase returned 0 comments"
                print(f"  Falling back to GraphQL: {selenium_fallback_reason}")
        except Exception as exc:
            selenium_fallback_reason = str(exc)
            print(f"  SeleniumBase failed, falling back to GraphQL: {selenium_fallback_reason}")

    if not comments:
        comments, post_info = fetch_comments(feedback_id, cookies=cookies)
        if post_info is None:
            post_info = {}
        post_info["comment_source"] = "graphql"
        if selenium_fallback_reason:
            post_info["selenium_fallback_reason"] = selenium_fallback_reason

    total_replies = 0

    for comment in comments:
        print(f"    Comment: {comment.get('text', '')[:50]}...")
        comment["replies"] = fetch_replies(comment, cookies=cookies)
        total_replies += len(comment["replies"])

        for reply in comment["replies"]:
            print(f"       Reply: {reply.get('text', '')[:50]}...")

        comment_clean = {k: v for k, v in comment.items() if not k.startswith('_')}
        all_data.append(comment_clean)

    if post_info is None:
        post_info = {}

    post_info["scraped_top_level_comments"] = len(all_data)
    post_info["scraped_reply_comments"] = total_replies
    post_info["scraped_total_comments"] = len(all_data) + total_replies

    print(f"  Found {len(all_data)} top-level comments and {total_replies} replies")
    return all_data, post_info
...
def scrape_simple_post():
    ...
    comments, post_info = fetch_comments_for_post(post_id, post_url=post_url)
